...
const Navbar = () => {
  ...
  return (
    ...
    <nav className={`${styles.paddingX} w-full flex  items-centre py-5 fixed top-0 z-20 bg-primary `}>
      <div className='w-full flex flex-row justify-between items-centre max-w-10xl mx-auto'>
        <Link to='/' className='flex items-centre gap-2'
        onClick={()=>{
          setActive("");
          window.scrollTo(0,0);
        }}>
          <img src={logo} alt="logo" className='w-9 h-9 object-contain' />
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Kanishk Raghav &nbsp; <span className='sm:block hidden '>| Web Developer</span></p>
        </Link>

        {/* <div className='flex-row cursor-pointer space-x-5  ' >
